...
- `src/services/sync/file/`: `provider.ts` (`FileSyncProvider`/`FileEntry`/`FileHead`/`FileSyncError{code:AUTH_FAILED|NOT_FOUND|NETWORK|CONFLICT|UNKNOWN,status?}`), `layout.ts` (de-WebDAV'd `SYNC_*` consts; paths pure), `wire.ts` (`RemoteBookConfig`/`RemoteLibraryIndex` + build/parse; `writerVersion:'wellread-webdav-1'` FROZEN), `merge.ts` (`mergeNotes`/`mergeBookConfig`/`mergeBookMetadata`/`isRemoteBookMetadataNewer` + law tests), `localStore.ts` (`LocalStore` iface), `appLocalStore.ts` (`createAppLocalStore({appService,settings,envConfig})` — the shared bridge that killed the duplicated buffered+streaming loaders), `engine.ts` (`FileSyncEngine(provider,store)` class + free `deleteRemoteBookDir(provider,hash)`), `index.ts`.
