...
describe('settingsAdapter', () => {
  ...
  test('pack only emits whitelisted fields, dropping non-whitelist keys', () => {
    ...
    expect(fields['alwaysOnTop']).toBeUndefined();
  ...
  test('pack flattens dot-namespaced nested keys', () => {
    const record: SettingsRemoteRecord = {
      name: 'singleton',
      patch: {
        readwise: { baseUrl: 'https://readwise.example' },
      } as Partial<SystemSettings>,
    };
    const fields = settingsAdapter.pack(record);
    expect(fields['readwise.baseUrl']).toBe('https://readwise.example');
  });
  ...
  test('pack ∘ unpack round-trips readwise / hardcover credentials', () => {
    const record: SettingsRemoteRecord = {
      name: 'singleton',
      patch: {
        readwise: { accessToken: 'rw-token', baseUrl: 'https://readwise.example' },
        hardcover: { accessToken: 'hc-token' },
      } as unknown as Partial<SystemSettings>,
    };
    const fields = settingsAdapter.pack(record);
    expect(fields['readwise.accessToken']).toBe('rw-token');
    expect(fields['readwise.baseUrl']).toBe('https://readwise.example');
    expect(fields['hardcover.accessToken']).toBe('hc-token');

    const out = settingsAdapter.unpack(fields);
    expect(out.patch.readwise?.accessToken).toBe('rw-token');
    expect(out.patch.readwise?.baseUrl).toBe('https://readwise.example');
    expect(out.patch.hardcover?.accessToken).toBe('hc-token');
  });

  test('declares encryptedFields covering readwise / hardcover credentials only', () => {
  ...
  test('readwise.baseUrl is plaintext (not in encryptedFields)', () => {
    expect(settingsAdapter.encryptedFields).not.toContain('readwise.baseUrl');
  });
  ...
  test('unpackRow reconstructs the patch from CRDT envelopes', () => {
    ...
    const row = makeRow({
      ...
      'readwise.baseUrl': env('https://readwise.example'),
    });
...
describe('SETTINGS_WHITELIST', () => {
  ...
  test('includes readwise and hardcover credential paths', () => {
    expect(SETTINGS_WHITELIST).toContain('readwise.baseUrl');
    expect(SETTINGS_WHITELIST).toContain('readwise.accessToken');
    expect(SETTINGS_WHITELIST).toContain('hardcover.accessToken');
  });
