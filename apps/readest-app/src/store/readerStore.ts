...
export const useReaderStore = create<ReaderStore>((set, get) => ({
  ...
  initViewState: async (
    ...
    try {
      ...
      // Failures must not block opening the book — run in background.
      void import('@/services/wellread/extract/ensureExtractForOpenedBook')
        .then(({ ensureExtractForOpenedBook }) =>
          ensureExtractForOpenedBook({ appService, book, bookDoc }),
        )
        .catch((e) => {
