...
export type GlobHit = { path: string };
export type GrepHit = { path: string; line: number; text: string };

function wellreadRoot(booksRoot: string): string {
  return normalizeAbsolute(`${booksRoot}/${WRITABLE_DIR}`);
}

function toWorkspacePath(hostPath: string, booksRoot: string): string {
...
function resolveSearchRoot(patternOrPath: string, booksRoot: string): string {
 ...
 */
export function globWellread(booksRoot: string, pattern: string): GlobHit[] {
  const root = normalizeAbsolute(booksRoot);
...
export type GrepOptions = {
  /** Workspace path or relative under .wellread; defaults to entire .wellread. */
  path?: string;
  /** Treat pattern as literal substring when false (default true = RegExp). */
  regex?: boolean;
  maxHits?: number;
};
