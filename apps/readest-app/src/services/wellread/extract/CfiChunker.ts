...
/**
 * CFI-aware chunker for wellread extract trees.
 * Walks an EPUB section's DOM via TreeWalker, accumulates text from <body>,
 * and slices it into ~maxChunkSize windows with paragraph > sentence > word
 * break-points. Each chunk carries epubcfi(/6/N!/…) anchors for goTo(cfi).
 */

/** Minimal chunk shape used by extract (no embedding fields). */
...
function buildChunks(
  ...
  for (const w of windows) {
    ...
    try {
      ...
      console.warn('[wellread] chunk_cfi_mismatch: failed to build range', err);
      continue;
    }
    ...
    try {
      ...
      console.warn('[wellread] chunk_cfi_mismatch: fromRange threw', err);
      continue;
    }
    ...
    if (!verifyRoundTrip(doc, startInner, startPair) || !verifyRoundTrip(doc, endInner, endPair)) {
      console.warn('[wellread] chunk_cfi_mismatch: CFI failed round-trip verification', {
        sectionIndex,
        position,
      });
      continue;
